    ...
    ($s:ident, $c:expr, $count:expr, $output:expr, $size:expr, $args:expr, ($( $vars:ident )*), ($( $param:ident, )*)) => {{
        ...
        if $c == 0 {
            ...
            let mut argv: Vec<String> = {
                // Arma may provide a pointer array aligned to 4 bytes even in the
                // 64-bit process. Read each pointer explicitly without assuming
                // the array has Rust's natural pointer alignment.
                let args = $args.unwrap();
                let mut argv = (0..$c)
                    .map(|index| std::ptr::read_unaligned(args.add(index)))
                    .map(|s| {
...
macro_rules! factory_tuple ({ $c: expr, $($param:ident)* } => {
        ...
        $($param: FromArma,)*
    {
        ...
        unsafe fn call(&self, _: Context, _: &ArmaContextManager, output: *mut libc::c_char, size: libc::size_t, args: Option<*mut *mut i8>, count: Option<libc::c_int>) -> libc::c_int { unsafe {
            let count = count.unwrap_or_else(|| 0);
            if count != $c {
                return format!("2{}", count).parse::<libc::c_int>().unwrap();
            }
            if $c == 0 {
                ...
                #[allow(unused_variables, unused_mut)]
                let mut argv: Vec<String> = {
                    // Arma may provide a pointer array aligned to 4 bytes even in
                    // the 64-bit process. Avoid creating an unaligned Rust reference.
                    let args = args.unwrap();
                    let mut argv = (0..$c)
                        .map(|index| std::ptr::read_unaligned(args.add(index)))
                        .map(|s| {
    ...
    {
        #[allow(non_snake_case)]
        unsafe fn call(&self, context: Context, _: &ArmaContextManager, output: *mut libc::c_char, size: libc::size_t, args: Option<*mut *mut i8>, count: Option<libc::c_int>) -> libc::c_int { unsafe {
    ...
    {
        #[allow(non_snake_case)]
        unsafe fn call(&self, _: Context, acm: &ArmaContextManager, output: *mut libc::c_char, size: libc::size_t, args: Option<*mut *mut i8>, count: Option<libc::c_int>) -> libc::c_int { unsafe {
    ...
    {
        #[allow(non_snake_case)]
        unsafe fn call(&self, context: Context, acm: &ArmaContextManager, output: *mut libc::c_char, size: libc::size_t, args: Option<*mut *mut i8>, count: Option<libc::c_int>) -> libc::c_int { unsafe {
            crate::RVExtensionFeatureFlags = FeatureFlags::default().with_context_stack_trace(false).as_bits();
            let call_context = acm.request().into_without_stack();
