...
if (_command in _directCommands || {(_command find "fuel:") == 0} || {(_command find "transport:") == 0}) exitWith {
...
if ((count _argumentsJson) > _requestChunkSize) then {
    ["stage", _command, _argumentsJson, _requestChunkSize, _callExtension] call FUNC(transport) params [
        "_stagedCommand",
        "_stagedArguments",
        "_stageSuccess"
    ];
...
["assemble", _payload, _success, _chunkPrefix, _chunkPrefixLength, _callExtension] call FUNC(transport)
