...
- Server workflow: belongs in `arma/server/src/features`.
- Arma command surface: belongs in `arma/server/src/<command_group>.rs` and possibly `command.rs`.
- Persistence mechanism: belongs in `arma/server/src/persistence`.
...
```text
arma/server/src/persistence/repository.rs
```
