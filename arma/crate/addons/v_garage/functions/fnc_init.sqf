 ...
 *
 * Example:
 * [_uid] call forge_crate_v_garage_fnc_init;
 */
...
private _virtualGarage = if (GVAR(persistenceEnabled)) then {
    ["v_garage:init", [_uid, toJSON _unlocks]] call EFUNC(extension,call) params ["_result", "_success"];
...
if (!(_virtualGarage isEqualType createHashMap) || { _virtualGarage isEqualTo createHashMap }) exitWith {
    ERROR("Virtual garage init returned an invalid payload");
    [SRPC(locker,initPlayer), [_uid]] call CFUNC(localEvent);
    createHashMap
};
