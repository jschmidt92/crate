 ...
 *
 * Description:
 * Constructs the Commander service object responsible for AI force spawning, threat assessment,
 * objective targeting, virtualization, and group maintenance.
 *
 * Arguments:
 * 0: [HASHMAP] - Mission configuration overrides (default: empty hashmap)
 *
 * Return Value:
 * Commander service object [HASHMAP OBJECT]
 *
 * Example:
 * [_config] call forge_crate_commander_fnc_create;
 */
...
GVAR(base_service) = createHashMapFromArray [
    ...
    ["ensureObjectives", {
        ...
        _objectives = [] call FUNC(objectives);
    ...
    ["battleLoop", {
        ...
        if (time - (_self get "lastReassignAt") > (_cfg getOrDefault ["minReassignInterval", 60])) then {
            [_self, _target] call FUNC(reassign);
    ...
    ["spawnerLoop", {
        ...
        private _enemySide = [] call FUNC(enemySide);
